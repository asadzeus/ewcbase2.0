...
import { GetSelledHouses } from './utils/utils';


const Home = () => {

  const [selledHouses, setSelledHouses] = useState([]);
  const [foundCount, setFoundCount] = useState(0);
  const [coords, setCoords] = useState([51, 0]);


  useEffect(() => {
    GetSelledHouses(setSelledHouses);
  }, []);

  useEffect(() => {
    if (selledHouses) {
      console.log("selled houses: ", selledHouses);
      setFoundCount(selledHouses.length);
    }
  }, [selledHouses]);
  ...
  return (
    <div className="home flex justify-center items-center h-[calc(100vh-100px)]">
      <div className='left flex-1 flex flex-col gap-4 justify-center items-center'>
        <div className="leftContainer w-[90%] h-[550px] flex flex-col gap-6 p-4">
          ...
          <div className="searchbar flex justify-around items-center h-[40px] w-full rounded-md gap-1">
            ...
            <input id='addressInput' type="text" name="" placeholder='Search Property Address' className='p-2 h-[100%] w-full border-1 rounded-md' />
          ...
          <div className="foundSlider flex justify-start items-center gap-10  ">
            ...
            {Array.isArray(selledHouses) && selledHouses.map((item, index) => (
              <a key={index} href={"/post/" + item.postId}><PostItem id={item.postId} price={item.price} address={item.address} bedroom={item.bedroom} bathroom={item.bathroom} area={item.area} /></a>
            ))}
      ...
      <div className='right flex-1 flex justify-center items-center h-full'>{
        selledHouses ?
          <Map position={coords} zoom={13} style="w-[550px] h-[550px] rounded-xl border-2 border-black" markers={selledHouses} />
          :
          <div>Loading map...</div>
      }
      </div>
